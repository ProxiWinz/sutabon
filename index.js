...
bot.on("ready",async() => {console.log(`${bot.user.username} is online !`); bot.user.setGame("How To Be Cwute + .cmds for help");});
bot.on("message", async message => {
   ...
   if(cmd === `${prefix}cwute`){
     return message.channel.send("I am the Goddess of Cwuteness!");
   }
})
bot.on("message", async message => {
   if(message.author.bot) return;
   ...
   if(cmd === `${prefix}lewd`){
     return message.channel.send("https://tenor.com/view/anime-lewd-gif-8681419");
   }
})
bot.on("message", async message => {
   if(message.author.bot) return;
   ...
   if(cmd === `${prefix}inv`){
     return message.channel.send("https://discordapp.com/api/oauth2/authorize?client_id=432359545344491522&permissions=8&scope=bot");
   }
})
bot.on("message", async message => {
   if(message.author.bot) return;
   ...
   if(cmd === `${prefix}fwuk`){
     return message.channel.send("Fwuk me,fwuk you , fwuk your neighbor, fwuk your mother, fwuk all of you!");
   }
})
bot.on("message", async message => {
   if(message.author.bot) return;
   ...
   if(cmd === `${prefix}trump`){
     return message.channel.send("https://twitter.com/realdonaldtrump")
   }
})
bot.on("message", async message => {
   if(message.author.bot) return;
   ...
   if(cmd === `${prefix}music`){
     return message.channel.send("https://www.youtube.com/watch?v=PeWtWxW3yZw&list=RDGMEMYH9CUrFO7CfLJpaD7UR85wVMPeWtWxW3yZw")
   }
})
bot.on("message", async message => {
   if(message.author.bot) return;
   ...
   if(cmd === `${prefix}order`){
     return message.channel.send("https://goo.gl/forms/JcxSnidgHexv76aF3")
   }
})
bot.on("message", async message => {
   if(message.author.bot) return;
   ...
   if(cmd === `${prefix}cmds`){
    return message.channel.send("```.order: Order a design | .inv: Invite me | .trump: See for yourself | .music: Presents a very hype playlist | .cwute : shows who is the cwutest | .lewd: For when someone is naughty | .fwuk: For when you get mad | .die: When you are at that point | .notwork: That's not is works you little shit ```");
   }
})
bot.on("message", async message => {
   if(message.author.bot) return;
   ...
   if(cmd === `${prefix}notwork`){
    return message.channel.send("http://i0.kym-cdn.com/photos/images/list/001/181/692/2bd.png");
   }
})
bot.on("message", async message => {
   if(message.author.bot) return;
   ...
   if(cmd === `${prefix}die`){
    return message.channel.send("Kill me now please. I SAID KILL ME. Alright fine, I'll do it myself. CYA LATER BITCH!!!");
   }
})
